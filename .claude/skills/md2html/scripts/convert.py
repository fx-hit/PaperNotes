...
CSS = r"""
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .image-lightbox {
    ...
    background: rgba(9, 12, 20, 0.86); padding: 84px 28px 34px;
  }
  ...
  .image-lightbox__image {
    ...
    margin: auto; border-radius: 8px; background: #fff;
  ...
  .image-lightbox__toolbar {
    ...
    background: rgba(255, 255, 255, 0.94); border: 1px solid rgba(225, 228, 232, 0.75);
    border-radius: 8px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.24);
  }
  .image-lightbox__button {
    width: 34px; height: 34px; border: 1px solid var(--border); border-radius: 6px;
    background: #fff; color: var(--heading); font: 700 17px/1 "Inter", sans-serif;
    display: inline-flex; align-items: center; justify-content: center;
    cursor: pointer;
  }
  .image-lightbox__button:hover { background: #f6f8fa; }
  .image-lightbox__zoom {
    min-width: 58px; text-align: center; color: var(--heading);
    font: 600 13px/1 "Inter", sans-serif;
  }
  .image-lightbox__caption {
    position: absolute; left: 28px; right: 28px; bottom: 12px;
    color: rgba(255, 255, 255, 0.86); text-align: center; font-size: 13px;
  ...
  @media (max-width: 1024px) {
    ...
    .image-lightbox { padding: 78px 14px 30px; }
    .image-lightbox__toolbar { top: 12px; }
  }
"""
...
(function() {
  ...
  overlay.innerHTML = `
    <div class="image-lightbox__toolbar" aria-label="图片缩放控制">
      <button type="button" class="image-lightbox__button" data-action="zoom-out" aria-label="缩小" title="缩小">-</button>
      <button type="button" class="image-lightbox__button" data-action="zoom-reset" aria-label="重置缩放" title="重置缩放">1x</button>
      <button type="button" class="image-lightbox__button" data-action="zoom-in" aria-label="放大" title="放大">+</button>
      <span class="image-lightbox__zoom" aria-live="polite">100%</span>
      <button type="button" class="image-lightbox__button" data-action="close" aria-label="关闭" title="关闭">&times;</button>
    </div>
  ...
  const zoomLabel = overlay.querySelector('.image-lightbox__zoom');
  ...
  function computeFitSize() {
    ...
    fitWidth = Math.max(1, naturalWidth * fitScale);
    applyZoom();
  }
  ...
  overlay.addEventListener('click', function(event) {
    ...
    if (action) {
      ...
      if (type === 'zoom-in') setZoom(zoom + step);
      if (type === 'zoom-out') setZoom(zoom - step);
      if (type === 'zoom-reset') setZoom(1);
