...
![图3：可扩展性对比](assets/llada/flops_mmlu_scatter.jpg)

*图3（共6个子图）：LLaDA 与自回归基线（ARM）在 6 个任务上随预训练 FLOPs 增加的性能变化。横轴为预训练计算量（FLOPs），纵轴为各项任务指标。*
...
*图3（完整6子图）：在 10²⁰ ∼ 10²³ FLOPs 范围内，LLaDA（蓝色）与自回归模型（橙色）的随计算量增长的趋势对比。关键发现：LLaDA 展现了与 ARM 竞争性的可扩展性，在 MMLU 和 GSM8K 上甚至表现出更强的扩展趋势。在 PIQA 等相对较弱的任务上，性能差距也随规模增加而缩小。*

**关键发现**：LLaDA 在 10²⁰ ∼ 10²³ FLOPs 范围内展现了与 ARM 高度竞争的可扩展性。在 MMLU 和 GSM8K 上，LLaDA 甚至表现更好。

### 4.2 基座模型 Benchmark（LLaDA 8B Base）

![图1a：基座模型零/少样本 Benchmark](assets/llada/LLaDA_vs_LLaMA.jpg)

*图1 (a)：LLaDA 8B Base 与 LLaMA3 8B Base、LLaMA2 7B Base 在 15 个零/少样本任务上的对比。LLaDA 在超半数任务上达到与 LLaMA3 8B 竞争的水平，尤其在数学（GSM8K 70.3 vs 48.7）和中文（CMMLU 69.9 vs 50.7）上大幅领先。*
...
![图1b：Instruct 模型 Benchmark](assets/llada/LLaDA_vs_LLaMA_chat.jpg)

*图1 (b)：LLaDA 8B Instruct（仅SFT）与 LLaMA3 8B Instruct（SFT+RL）的对比。LLaDA 在仅使用 SFT 的情况下，多项指标与经过 RL 对齐的 LLaMA3 8B Instruct 接近。*
...
![采样效率分析](assets/llada/GSM8K_efficiency.jpg)

*图：采样效率分析——LLaDA 在不同采样步数（32/64/128/256 步）下的速度-质量权衡。横轴为吞吐量（tokens/s），纵轴为任务指标。*
...
- **纯自回归**：Base 模型可用，但 Instruct 模型效果差（SFT 数据是完整句子）
