...
Joint WAM 把未来视频 token 和动作 token 放进同一个 diffusion / flow matching 过程里，通过共享注意力让两者彼此条件化。DreamZero、Motus、Fast-WAM-Joint、X-WAM 都属于或接近这一类。

![图3：HarmoWAM 动机实验揭示两类 WAM 范式的互补失败模式](assets/worldmodel-wam/harmowam-motivation.jpg)
