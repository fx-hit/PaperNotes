...
*图1：X-WAM 整体概览，分为上半部分（方法架构）和下半部分（三项核心实验结果）。*

**上半部分——方法架构流程**：从左到右展示了 X-WAM 的完整推理管线。输入端接收三类信号：(1) 语言指令 $c$（如"把耳机放入盒子"），(2) 多视角初始 RGB 观测 $O_0$（来自全局相机和腕部相机的多个视角），(3) 机器人当前本体感受状态 $s_0$（双机械臂末端位姿和夹爪位置）。这些条件信号经过 VAE 编码和 MLP 投影后，送入基于 Wan2.2-5B 初始化的 Diffusion Transformer 联合去噪。架构中的两个核心创新标注在关键位置：**Lightweight Depth Adaptation**（浅蓝色标注）——在 DiT 末端复制最后几层 block 构造并行深度分支，通过单向交叉注意力从主分支提取空间信息，输出深度图用于 3D 重建；**Asynchronous Noise Sampling**（橙色标注）——对视频和动作施加不同的去噪时间步，动作在先、视频在后，实现高效的异步推理。最终输出包括：(1) 未来多视角 RGB-D 视频 $O_{1:H}, D_{1:H}$（$H=8$ 帧），(2) 未来本体感受状态 $s_{1:H}$，(3) 未来动作序列 $a_{1:K}$（$K=32$ 步，控制频率为视频帧率的 4 倍）。各视角深度图通过预测的末端执行器位姿融合为统一的 3D 点云。

**下半部分左侧——策略成功率对比（柱状图）**：对比 X-WAM 与所有基线在 RoboCasa（24 个厨房任务）和 RoboTwin 2.0（50 个双臂任务，含 Clean 和 Randomized 两种设置）上的平均成功率。横轴为不同模型，纵轴为成功率（%）。RoboCasa 上 X-WAM 达到 **79.2%** 的最高柱，远超 Cosmos Policy 的 67.1%、GR00T-N1.5 的 64.1% 和 $\pi_0$ 的 62.5%。RoboTwin 2.0 上 Clean 设置 X-WAM 为 **89.8%**，Randomized 设置为 **90.7%**，均高于 Motus（88.7%/87.0%）和 GigaWorld-Policy（87.0%/85.0%）。注意 Randomization 下 X-WAM 的成功率反而略高于 Clean（90.7% vs 89.8%），说明显式 3D 空间感知使模型对视觉外观变化具有极强的鲁棒性——随机化纹理和干扰物反而让模型更依赖几何结构而非表面纹理做决策。

**下半部分中间——4D 重建质量对比（雷达图/多指标图）**：展示了 X-WAM 与 DreamZero+DA3、Robot4DGen、X-WAM w/o depth + DA3 在 6 个重建指标上的对比。指标涵盖三个方面：RGB 视觉保真度（PSNR、SSIM、LPIPS）、深度精度（AbsRel、$\delta_1$）、和点云质量（Chamfer Distance）。X-WAM 在所有 6 个维度上包围其他方法，尤其在 Chamfer Distance 上以 0.0049 大幅领先（vs DreamZero+DA3 的 0.0680，**差距超过一个数量级**），证明端到端深度预测在几何一致性上远优于后处理方案。

**下半部分右侧——真机部署照片**：展示 X-WAM 部署在 AC One 双臂机器人平台执行耳机包装任务的真实场景。机器人需要完成四阶段序列操作：抓取盒子开盖→放入第一只耳机→放入第二只耳机→合盖归位。这个任务对 3D 空间精度要求极高——插入耳机的间隙仅亚厘米级——使得纯 2D 模型（论文中报告 $\pi_{0.5}$ 在此任务上完全失败，进度仅 25%–50%）无法完成。X-WAM 通过 8 步异步去噪 + RTC 实现约 300ms 延迟的实时推理，成功完成完整任务流程。
