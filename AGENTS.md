...
```
notes/2026-05/2026-05-21/DreamZero_阅读笔记.md
notes/2026-05/2026-05-21/DreamZero_阅读笔记.html
```
...
4. **更新站点入口** — 日常维护优先运行 `./scripts/build_note.sh <笔记.md>`；它会转换 HTML、运行 `./generate_index.sh --site` 并执行全仓审计。只更新入口页时运行 `./generate_index.sh --site`。
   ...
   - `generate_index.sh` 仅扫描日期目录**顶层**（`notes/20??-??/20??-??-??/*.html`），因此笔记 HTML 必须放在 `notes/YYYY-MM/YYYY-MM-DD/` 目录下，不能放在更深的子目录中。
...
```
notes/20??-??/20??-??-??/<repo-name>/
```

该前缀只匹配 `notes/YYYY-MM/YYYY-MM-DD/` 下与笔记相邻的代码仓库，不会误伤 `assets/` 下同名的子目录。例如：

```
notes/20??-??/20??-??-??/dreamzero/
notes/20??-??/20??-??-??/FastWAM/
notes/20??-??/20??-??-??/giga-world-policy/
```
