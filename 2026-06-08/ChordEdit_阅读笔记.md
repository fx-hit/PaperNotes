...
*图 1：ChordEdit 基于 SD-Turbo（上两行）和 SwiftBrush-v2（底行）的跨类型编辑效果总览。*

- **编辑类型覆盖**：从左到右包含物体替换（"man → woman"、"llama → giraffe"）、属性编辑（"sunglasses"、"red hair"、"smiling"）、风格迁移（"oil painting"、"comic style"）、场景变换（"snow"、"underwater"）等 12 种语义变化。
- **编辑精确度**：目标语义精准命中——"astronaut" 的宇航头盔细节、"snow" 场景的自然雪地环境均高质量生成。
- **非编辑区域保持**：背景、光照、姿态、身份等几乎无变化——这是训练免/反转免单步方法此前无法实现的。
- **跨模型一致性**：SD-Turbo（扩散蒸馏）和 SwiftBrush-v2（一步生成）上均表现一致，验证模型无关性。
...
![图2：单步 Simple Drift 编辑的失败模式](assets/chordedit/naive_show.jpg)

*图 2：Simple Drift 在单步下的两种典型失败模式及 ChordEdit 修正。*

- **(a) 物体畸变**：三组案例（"cat→dog"、"closed eyes→open eyes"、"grass→beach"）中 naive 方法导致结构崩塌——狗的头部纹理撕裂、眼睛区域错乱、背景伪结构浮现。底部裁剪放大区清晰展示了纹理错乱 vs ChordEdit 的自然保真。
- **(b) 背景坍塌**：非编辑区域出现不自然的纹理混合、轮廓模糊——天空、地面等产生非物理的碎裂感。ChordEdit 在同样单步下保持背景完整。
- **失败机制**：Naive 场 $\Delta v$（两高能量场之差）在 $h=1$ 大步长下累积大量局部截断误差，轨迹大幅偏离 OT 最优路径。
...
- $u_t(x)$：在时刻 $t$、位置 $x$ 处，驱动概率质量移动的**速度向量场**。它就是论文要找的**编辑控制场**——告诉每一个点"你应该往哪个方向、以多大速度移动"。
- $\frac{1}{2}\|u_t(x)\|^2$：该点此刻的动能（物理学中，质量为 $m$ 速度为 $v$ 时动能 = $\frac{1}{2}mv^2$，这里质量就是概率密度 $\rho$）。
- $\iint \frac{1}{2}\|u\|^2\rho\,dxdt$：整个传输过程从 $t=1$ 到 $t=0$ 的总动能。目标是最小化它——找到最"省力"的传输路径。
...
- **编辑过程**：找到一个向量场 $u_t(x)$，它的积分路径能把 $x_{\text{src}}$ 从 $t=1$ 传输到 $t=0$ 且到达目标分布，同时总动能最小。

这个视角与 naive 方法的根本区别是：
- **Naive**：源场和目标场各算各的（都很大），然后做差——结果还是很大、很乱。
- **OT**：直接要求整个传输过程中场的总能量最小——这本身就是"平滑稳定"的数学保证。
...
- **(c) ChordEdit**：用 $\mathbf{R}(x_\tau, t)$ 和 $\mathbf{R}(x_\tau, t-\delta)$ 的时间加权平均构造平滑 Chord Field（红色箭头），该场天然稳定，单步内精确从 $x_{\text{src}}$ 到达 $x_{\text{tar}}$。时间平滑抑制了高能量尖峰，降低了 Lipschitz 常数，显著减小单步积分误差。
...
**观测噪声假设**：$\mathbf{R}(x_\tau, t) = u_t(x_\tau) + \varepsilon_t$，其中 $\mathbb{E}[\varepsilon_t] = 0$。即我们"观测"到的 $\mathbf{R}$ 是被零均值噪声污染的 $u_t$。这不是一个强假设——它只是说观测值围绕真实值对称地波动，没有系统性偏差。这个假设非常弱且合理。

#### 第二步：MAP 估计——如何从含噪观测得到平滑估计？

> 现在有了观测 $\mathbf{R}$，但它含噪声、能量高、不稳定。我们需要一种方法从含噪数据中提取一个"干净版本"。
>
> 一个自然的思路来自统计学中的**最大后验估计（MAP, Maximum A Posteriori）**：我们想要一个既接近观测（数据保真度）、又不能偏离之前估计太远（平滑先验）的折中解。具体做法是：
...
**直观解读**：最优估计 $u_t^\star$ 是**前一估计 $\hat{u}_{t-\delta}$ 和窗口内平均观测 $\frac{1}{\delta}\int\mathbf{R}$ 的加权平均**（注意第二项是 $\frac{\delta}{t+\delta}$ 乘以窗口观测的均值）。当 $t$ 大（编辑早期），我们更信任之前的估计（平滑性强）；当 $\delta$ 大（窗口宽），更多新观测被纳入，估计更忠实于数据。
...
**为什么只需要两次模型查询？** 因为 $\mathbf{R}(t)$ 和 $\mathbf{R}(t-\delta)$ 分别在时间 $t$ 和 $t-\delta$ 通过共享噪声的含噪代理计算。由于只需要两个时间点，它们可以在**同一个 batch** 内并行查询 UNet——这就是"1 NFE transport"的实现原理。

![图4：2D 合成分布传输——Chord vs Naive](assets/chordedit/edit_curve_triptych.jpg)
...
- **关键结论**：$\mathcal{C}_{\text{cho}} \le \mathcal{C}_{\text{nai}}$。由于 Chord 场的三项 $L^\infty$ 范数都被收缩了，一致性常数也就更小 → Euler 步的误差更小 → 单步 $h=1$ 下 ChordEdit 比 naive 更精确。
- **全局误差**：论文进一步证明（Theorem C.6），对整个编辑过程（$t=1\to 0$）的累积误差，ChordEdit 的上界严格低于 naive。这就是为什么 ChordEdit **可以**在 $h=1$ 单步下正常工作而 naive 不能的数学原因。

#### (4) 方差缩减（为什么 $n=1$ 就够）

- **背景**：传统方法需要多个噪声样本（$n>1$）来降低估计方差（MC 平均：方差 $\propto 1/n$）。
- **Chord 的降方差机制**：把观测模型 $\mathbf{R} = u^\star + \eta$（$\eta$ 是噪声）代入 Chord 公式 $\hat{u} = K_\delta * \mathbf{R}$。卷积 $K_\delta * \eta$ 把噪声 $\eta$ 的方差从 $\sigma^2$ 缩减为 $\|K_\delta\|_{L^2}^2 \sigma^2 \propto \delta^{-1}\sigma^2$（$\|K_\delta\|_{L^2}^2$ 是平滑核的"集中度"，$\delta$ 越大方差降得越多）。
- **代价**：引入了一个 $O(\delta^2)$ 的偏差（因为平滑会抹掉真实场的细节）。但合理选择 $\delta$ 时，偏差增长远小于方差下降——净 MSE（均方误差 = 偏差² + 方差）显著降低。
- **直接推论**：既然核平滑已经降了一大截方差，再叠加 MC 多噪声平均的边际收益就微乎其微了。这就是为什么 $n=1$ 就足够——图 10-11 的实验完美验证了这一点。
...
**通俗理解**：transport 已经把你安全送到了"目的地附近"，prox 是做最后的一步微调——用目标 prompt 在中等噪声水平 $t_c=0.30$ 下做一次单步去噪。由于噪声水平不高（$\sigma_{0.30}$ 适中），这一步对背景的改变较小，但能让目标语义（颜色、纹理、风格）更鲜明。
...
*图 5：四组编辑案例对比 Transport Only（NFE=1）vs Transport + Prox（NFE=2），验证分离式设计的合理性。*

- **Transport Only**：已完成主要语义转换（苹果→橙色、猫→虎纹），但语义强度保守（颜色过渡柔和、纹理变化不极致）。
- **Transport + Prox**：精炼显著增强语义（橙色更饱满、虎纹更鲜明、秋叶颜色更浓郁），同时保持 transport 阶段的结构约束（背景、姿态、光照不变）。
...
![图6：PSNR vs CLIP-Edited vs Runtime 三维对比散点图](assets/chordedit/sota_psnr_clip.jpg)
...
*图 7：多步/少步/单步方法的编辑结果逐行对比。第一列为原始输入。*

- **多步方法**：语义对齐好但常伴随非编辑区域变化（背景色偏移、结构微量扭曲）——迭代误差累积虽小但可见。
- **少步方法**：背景保持最强（PSNR 最高），但语义对齐偏保守（编辑变化不够显著）。
- **ChordEdit**：在语义强度（"fire"的火焰、"oil painting"的笔触）和背景保真（地面纹理、建筑结构、光线）之间取得最优视觉平衡。
...
*图 8：编辑场能量定性可视化与编辑效果对比。三列分别基于 SwiftBrush-v2、SD-Turbo、SD-Turbo。每列的 Naive 场能量热力图（亮白区域 = 高能量）对应严重的伪影和背景碎裂；Chord 场能量热力图整体暗色、均匀分布在编辑区域，对应干净、自然的输出。核心：低能量 ≈ 高质量。*
...
![图11：不同噪声样本数 n 的编辑结果定性对比](assets/chordedit/noise_show.jpg)

*图 11：$n=1,2,3,4$ 的 ChordEdit 输出（$\delta=0.15$，其余参数固定）。四列在视觉上几乎无法区分——物体语义、背景一致性、细节纹理在所有 $n$ 下高度一致，从定性角度确认了图 10 的定量结论。*
...
*图 12：固定其他参数、变化 $\delta$ 的编辑结果。*

- **$\delta=0.00$（naive）**：全部案例崩溃——"tiger→cat"脸崩、"apple→cat"输出与目标无关、"man→woman"纹理扭曲。
- **$\delta=0.05$**：立即出现质变——猫脸恢复、女性特征出现。即使微小平滑（0.05）也对稳定性产生显著作用。
- **$\delta=0.10/0.15/0.20$**：逐步增强平滑。$\delta=0.15$ 达到稳定性-语义最优平衡——物体清晰、背景无损、编辑明确。
- **$\delta=0.30$**：过度平滑导致编辑不足（语义变弱）。核心：$\delta>0$ 是"开关"——任一非零值即开启稳定；$\delta$ 大小调节平滑 vs 语义。

![图13：步长缩放 λ 的定性消融——作为直觉化编辑强度旋钮](assets/chordedit/ablation_show_ablation_lambda.jpg)

*图 13：固定其他参数、变化步长缩放 $\lambda$。"mountain → volcano" 案例。*

- **$\lambda=0.8$**：编辑极保守——山轮廓几乎不变、火山特征微弱。
- **$\lambda=1.0$**：火山形态出现（山顶熔岩、烟雾），默认参数。
- **$\lambda=1.2/1.4$**：火山特征强化（熔岩更显眼、山色更深），但背景保持性开始下降（天空色偏、地形变化）。
- **$\lambda$ 的角色**：直觉化"编辑强度旋钮"——用户无需理解内部机制即可在"保守"和"大胆"之间调节。
