...
> - 论文图片：`2026-06-11/arXiv-2602.22766v2/image*.pdf`、`image*.png`
...
### 3.3 Finding 1：X → Z 断开——Latent token 不受输入影响
...
### 3.4 Finding 2：Z → Y 断开——改了 latent token 答案几乎不变
...
### 3.5 Finding 3：Latent token 编码的视觉语义极少
...
### 5.4 CapImagine 的因果分析：文本想象是真正的因果变量
...
*图 5：Monet（latent）、CapImagine（text）和 DeepEyes（tool）三种方法在 V* benchmark 各子类上的推理速度对比，柱状图格式。*

*横轴为 V* benchmark 的不同子类别（Attribute / Spatial 等），每个类别包含三根并排柱形。纵轴为解码时间（Seconds），仅计解码阶段、不含图像编码和 prompt 处理时间。三种颜色对应三种方法——CapImagine（文本想象）、Monet（latent 想象）、DeepEyes（工具调用 + zoom-in 操作）。*
