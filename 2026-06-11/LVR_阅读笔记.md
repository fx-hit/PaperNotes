...
*图 2：LVR 训练和推理流程，两栏分别展示 SFT 和 RL 阶段，以及推理时的三种解码策略。*
...
### 7.1 多种 forward 模式的 Monkey Patch

LVR 不是修改 Qwen2.5-VL 源码，而是通过 monkey patch 替换 forward 函数。`replace_qwen2_5_with_mixed_modality_forward_lvr()` 根据参数组合切换不同 forward：

| 场景 | forward 函数 | 触发条件 |
|------|------------|---------|
| SFT 训练（无 Head） | `qwen2_5_mixed_modality_forward_lvr` | `inference_mode=False, lvr_head=False` |
| SFT 训练（有 Head） | `qwen2_5_mixed_modality_forward_lvr_with_head` | `lvr_head=True` |
| SFT + LatentEndToken | `*_with_latentEndToken` | `latent_end_token=True` |
| SFT + ModeSwitchLoss | `*_with_modeSwitchLoss` | `mode_switch_loss=True` |
| 推理（无 Head） | `qwen2_5_mixed_modality_forward_lvr_inference` | `inference_mode=True` |
| 推理（有 Head） | `*_with_head_inference` | `inference_mode=True, lvr_head=True` |
| RL GRPO | `qwen2_5_mixed_modality_forward_lvr_grpo` | `rl=True` |
...
Fixed Token 解码的关键是维护一个 `lvr_remaining_steps` 配额——进入 LVR 时重置为原始预算，每步递减，耗尽时自动退出。这个逻辑完全基于 counter，不依赖任何模型的内部状态判断，因此绝对稳定。

### 7.3 数值稳定性

Qwen2.5-VL 的 3D 卷积在 visual patch embedding 过程中可能产生 NaN（极端分辨率下）。代码通过 `monkey_patch_patch_emb.py` 把相关计算从 fp16/bf16 切换到 fp32，并注册 NaN 检测 hook。
...
1. **可变长度推理不可靠**：Fixed Token 虽然好用但显然不是终极方案。论文在 Latent End Token 和 Mode Switching Loss 上的负结果说明，continuous space 中定义停止条件仍然是一个开放问题。
2. **RL 只在 3B 上验证**：计算资源限制导致 7B 的 RL 效果未知。3B 上的 RL 提升虽稳健但不大（+1-2pp），7B 上的 RL 是否有同样/更强的提升尚未验证。
3. **多图像场景未覆盖**：Relative Reflectance 的表现暴露了单图像训练的限制。扩展到多图像需要重新设计 LVR 的推理目标。
4. **GRPO_latent 的工程复杂度**：训练时需要记录每个 rollout 的 hidden states 并在重放时精确 patch，增加了训练代码的复杂度和内存开销。
