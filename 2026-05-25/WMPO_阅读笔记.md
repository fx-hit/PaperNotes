...
![图1：三种 VLA 训练范式对比。从左到右分别展示了 (a) 模仿学习、(b) 真实环境 RL、和 (c) WMPO 的完整训练流程与数据流向。](assets/wmpo/introduction.jpg)
...
![图2：WMPO 训练流程总览。从初始状态 s₀ 出发，策略 π_θ_old 与世界模型 p_φ 交替交互生成想象轨迹，多条轨迹经奖励模型 R_ψ 评估后，通过 GRPO 更新策略参数。](assets/wmpo/method.jpg)
...
![图3：Square 任务上的行为序列对比。上排为基础策略的执行过程，下排为 WMPO 优化的策略。图中展示了从初始状态到最终结果的关键帧序列，清晰揭示了 WMPO 策略的自纠正能力。](assets/wmpo/behavior.jpg)
...
![图5：不同策略成功轨迹的相对平均长度对比（以 Base Policy 为 100% 基准）。横轴为方法名称，纵轴为相对轨迹长度百分比。数值越低表示策略越高效。](assets/wmpo/length.jpg)
...
![图6：StackThree 任务上的终身学习曲线。横轴为迭代轮次，纵轴为成功率。三条曲线分别对应 WMPO、DPO 和不同专家演示量（300/428/556）训练的 Base Policy。](assets/wmpo/lifelong.jpg)
...
![图7：真实机器人实验 —— "将方块插入立柱"，方块与立柱间隙仅 5mm。上排为基策略在真实世界中的执行过程（真实相机拍摄），下排为世界模型从同一初始状态出发的"想象"预测。两排序列展示了世界模型对物理动力学的惊人保真度。](assets/wmpo/real_success.jpg)
